...
/**
 * \file
 * \brief imx28 USB device controller driver head file
 *
 * \internal
...
#ifndef __AWBL_IMX28_USBD_H
#define __AWBL_IMX28_USBD_H
...
#define AWBL_IMX28_USBD_DRV_NAME	"awbl_imx28_usbd"

/** \brief imx28 USB device EndPoint Queue Head (dQH) */
struct awbl_imx28_usbd_qh {
	uint32_t	info1;
...
/** \brief imx28 USB device transfer descriptor (dDT) */
struct awbl_imx28_usbd_td {
	uint32_t					next;
...
struct awbl_imx28_usbd_ep {
	struct awbl_usbd_ep			ep;
	struct awbl_imx28_usbd		*p_usbd;
	struct awbl_imx28_usbd_qh	*p_qh;
...
/** \brief imx28 USB device controller struct */
struct awbl_imx28_usbd {
	struct awbl_usbd			usbd;

	/** \brief EPs (IN and OUT) */
	struct awbl_imx28_usbd_ep	*eps;
	struct awbl_imx28_usbd_qh	*qhs;
...
/** \brief imx28 USB device controller information */
struct awbl_imx28_usbd_info {
...
void awbl_imx28_usbd_drv_register (void);

#ifdef __cplusplus
}
#endif  /* __cplusplus  */

#endif /* __AWBL_IMX28_USBD_H */
