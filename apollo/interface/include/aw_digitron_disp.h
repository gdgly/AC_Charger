 ...
 * aw_digitron_disp_str(0, "0123");     // 数码管显示字符串"0123"
 * \endcode
 *
 * \internal
...
/**
 * \brief 8段数字型数码管ASCII字形
 *
 * \note 用户可根据该结构体自定义ASCII字形解码表
 */
struct aw_seg8_ascii_code {
 ...
 * @param p_tbl   解码表
 */
void aw_digitron_disp_decode_set (int id, uint16_t (*pfn_decode)(uint16_t code));
 ...
 * \note 能显示的字符由解码函数确定
 */
void aw_digitron_disp_str (int id, int index, int len, const char *p_str);
 ...
 * @param id    显示器编号
 */
void aw_digitron_disp_clr (int id);

/**
 * \brief 使能数码管显示，开启扫描（默认状态是开启的）
 * @param id    显示器编号
 */
void aw_digitron_disp_enable (int id);

/**
 * \brief 禁能数码管显示,停止扫描
 * @param id   显示器编号
 */
void aw_digitron_disp_disable (int id);
