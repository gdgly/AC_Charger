 ...
 * 了该非易失性信息的大小，则其行为未定义。
 *
 * \param p_name   非易失性存储信息的名字
 * \param unit     非易失性存储信息的单元号 (相同名字的非易失性信息用单元号区分)
 * \param p_buf    数据返回的缓冲区
 * \param offset   在非易失性存储信息中的偏移
 * \param len      要读取的长度
 *
 * \retval AW_OK   成功
 * \retval -EFAULT \a p_name 或 \a p_buf 为NULL
 * \retval -EXIO   指定的非易失性存储信息不存在或操作失败
 *
 * \par 示例
 * \code
 *  #include "aw_nvram.h"
