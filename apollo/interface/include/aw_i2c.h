 ...
 * \brief 设置I2C设备结构体参数
 *
 * \param p_dev  I2C从机设备描述符指针
 * \param busid  从机设备所属
 * \param addr   从机设备地址
 * \param flags  从机设备标志，见“I2C从设备属性标志”
 *
 * \par 范例
 * \code
 * #include "aw_i2c.h"
 ...
 * \brief 设置I2C传输结构体参数
 *
 * \param p_trans   I2C传输描述符指针
 * \param addr      目标器件地址
 * \param flags     传输控制标志，见“I2C传输控制标志”
 * \param p_buf     数据缓冲区
 * \param nbytes    传输数据个数
 *
 * \par 范例
 ...
 * \brief 设置I2C消息结构体参数
 *
 * \param p_msg             I2C消息描述符指针
 * \param p_transfers       组成消息的“I2C传输”
 * \param trans_num         “I2C传输”的个数
 * \param pfunc_complete    传输完成回调函数(仅调用 aw_i2c_async() 时此参数有效,
 ...
 *
 * \retval AW_OK        读取成功
 * \retval -ENXIO       未找到\a p_dev->masterid 所指定的主机
 * \retval -EFAULT      \a p_buf 为 NULL
 * \retval -AW_EINVAL   参数错误，\a nbytes 为 0
 * \retval -ENOTSUP     \a p_dev->flags 中某些特性不支持
 *                      (例如, #AW_I2C_IGNORE_NAK)
 * \retval -ETIME       超时
 * \retval -ENODEV      从机地址无应答(若\a p_dev->flags 设置了
 *                      #AW_I2C_IGNORE_NAK, 则不会产生此错误)
 * \retval -ENOENT      数据无应答(若\a p_dev->flags 设置了 #AW_I2C_IGNORE_NAK,
 *                      则不会产生此错误)
