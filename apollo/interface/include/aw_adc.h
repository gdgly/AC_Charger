 ...
 * \brief 转换完成回调函数类型
 *
 * p_cookie为用户初始化客户端传入的回调函数参数
 * state 转换完成的状态:
 *    AW_OK        : 转换成功
 *    AW_ERROR     : 转换出错
 *
 * 处理转换数据前应该确保转换结束的状态为AW_OK。如果不为AW_OK，则后续转换序列将
 * 自动取消，不会再转换。通常情况下，转换出错可能是由于缓冲区的buf过小，使得buf
 * 切换需要很频繁，数据处理时间稍长，数据就溢出了。可以增大缓冲区的buf大小以增
 * 加buf切换的时间间隔。
 */
 ...
 * \param[in] p_buf         用于存放ADC转换结果的缓冲区,注意数据类型，数据宽度
 ...
 *
 * \param[in] ch     AD通道号
 * \param[in] p_val  用于存放采样值的数组地址，具体类型与ADC的数据宽度相关
 * \param[in] cnt    需要采样的个数
 * \param[in] urgent 紧急转换任务，将优先于urgent为FALSE的转换序列转换
 *
 * \retval >=0    该AD通道的数字量值
 * \retval -ENXIO AD通道号不存在
 *
 *
 * \note 关于p_val值的类型
 *
 ...
 * 户端，则会在当前普通客户端的一个序列结束后，强制打断普通客户端的转换，切换到
 * 紧急客户端。
 */
 ...
 *
 * \note  关于缓冲区描述符，缓冲区描述符描述了存放ADC转换结果的缓冲区，可以是单
 * 个或多个，如果指定了多个缓冲区。则会在前一个缓冲区填充满后，自动连续的切换到
 * 下一个缓冲区。指定序列转换次数为多次时（包括持续转换(0)），缓冲区个数至少为2，
 * 如果为缓冲区个数为1，将返回参数错误（单个缓冲区无法循环填充）。
 */
