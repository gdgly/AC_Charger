 ...
 *
 * \param msgq  消息队列实体, 与 AW_MSGQ_INIT(), AW_MSGQ_SEND(),
 *              AW_MSGQ_RECEIVE() 和 AW_MSGQ_TERMINATE() 一起使用。
 *
 * \param msg_num  消息条数, 消息缓冲区大小 = \a msg_num * \a msg_size
 * \param msg_size 消息大小, 消息缓冲区大小 = \a msg_num * \a msg_size
 *
 * \par 范例：常规用法
 * \code
 *  AW_TASK_DECL_STATIC(task_a, 512);
 ...
 *
 * \param  msgq     消息队列实体,由 AW_MSGQ_DECL()或 AW_MSGQ_DECL_STATIC() 定义
 * \param  msg_num  消息条数, 消息缓冲区大小 = \a msg_num * \a msg_size
 * \param  msg_size 消息大小, 消息缓冲区大小 = \a msg_num * \a msg_size
 * \param  options  初始化选项标志, #AW_MSGQ_Q_PRIORITY 或 #AW_MSGQ_Q_FIFO
 *
 ...
 *     判断调用结果: AW_OK -成功获取消息；-ENXIO -在等待的时候，消息队列被终止
 *
 *   - #AW_MSGQ_NO_WAIT \n
 *     立即返回，不管是否成功获取到消息。通过返回值来判断调用结果:
 *     AW_OK -成功获取消息; -EAGAIN -未获取到消息, 稍后再试; -ENXIO -在等待的时
 *     候，消息队列被终止
 *
 ...
 *
 * \param   msgq     消息队列实体,由 AW_MSGQ_DECL()或 AW_MSGQ_DECL_STATIC() 定义
 * \param   p_buf    保存消息的缓冲区
 * \param   nbytes   保存消息的缓冲区的长度
 ...
 * \retval  -ETIME   获取消息超时
 * \retval  -ENXIO   在等待的时候, 消息队列被终止
 * \retval  -EAGAIN  获取消息失败, 稍后再试 (\a timeout = #AW_MSGQ_NO_WAIT 时)
 *
 * \par 范例
 ...
 *     判断调用结果: AW_OK -成功发送消息；-ENXIO -在等待的时候，消息队列被终止
 *
 *   - #AW_MSGQ_NO_WAIT \n
 *     立即返回，不管是否成功发送到消息。通过返回值来判断调用结果:
 *     AW_OK -成功发送消息; -TIME -发送超时; -ENXIO -在等待的时候,消息队列被终止
 *
 * 除了 #AW_MSGQ_WAIT_FOREVER 外，其它的负值都是非法的。调用者需要保证指定的超时
 * 值为正数。
 *
 ...
 *
 * \param   msgq    消息队列实体, 由AW_MSGQ_DECL() 或AW_MSGQ_DECL_STATIC() 定义
 * \param   p_buf   消息缓冲区
 * \param   nbytes  消息长度
 * \param   timeout 最大等待时间(超时时间)，单位是系统节拍，系统时钟节拍频率
 *                  由函数 aw_sys_clkrate_get()获得, 可使用 aw_ms_to_ticks()
 ...
 * \retval  -ETIME  发送消息超时
 * \retval  -ENXIO  在等待的时候, 消息队列被终止
 * \retval  -EAGAIN 发送消息失败, 稍后再试 (\a timeout = #AW_MSGQ_NO_WAIT 时)
 *
 * \par 范例
 ...
 * \brief 终止消息队列
 *
 * 终止消息队列。任何等待此消息队列的任务将会解阻塞，并返回 -ENXIO 。此函数不会
 * 释放消息队列相关的任何内存。
 *
 * \param   msgq     消息队列实体,由 AW_MSGQ_DECL()或 AW_MSGQ_DECL_STATIC() 定义
 *
 ...
 *
 * \param   msgq_id  消息队列ID,由 AW_MSGQ_INIT() 返回
 * \param   p_buf    保存消息的缓冲区
 * \param   nbytes   保存消息的缓冲区的长度
 * \param   timeout  最大等待时间(超时时间)，单位是系统节拍，系统时钟节拍频率
 *                   由函数 aw_sys_clkrate_get()获得, 可使用 aw_ms_to_ticks()
 ...
 * \brief 检查是否为有效消息队列
 *
 * \param   msgq_id 消息队列ID，由AW_MSGQ_INIT()返回
 *
 * \retval  TRUE    有效队列
 * \retval  FALSE   无效队列
 */
