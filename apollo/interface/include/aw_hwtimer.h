 ...
 * 特性值，则把该特性参数设为NULL
 *
 * \param timer         要查询的定时器句柄
 * \param p_min_freq    保存定时器最小频率的指针
 * \param p_max_freq    保存定时器最大频率的指针
 * \param p_features    保存定时器特性的指针
 *
 * \retval AW_OK    没有错误
 * \retval -EINVAL  无效的定时器句柄
 */
 ...
 *
 * \param freq      期望的定时器输入频率（频率越高精度越高）
 * \param min_freq  期望至少能达到的最小频率
 * \param max_freq  期望至少能达到的最大频率（硬件无倍频的情况下max_freq ≦ freq）
 * \param features  定时器特性
 * \param pfunc_isr 定时器中断服务函数
 * \param p_arg     定时器中断服务函数参数
 *
 ...
 * \brief 使能定时器
 *
 * \param timer        要使能的定时器句柄
 * \param frequency_hz 定时中断频率
 *
 * \return AW_OK 为成功，其它错误码为失败
 */
