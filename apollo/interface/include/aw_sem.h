 ...
 *
 * \param sem 信号量实体, 与 AW_SEMB_INIT(),  AW_SEMB_TAKE(), AW_SEMB_GIVE()
 ...
 * 宏定义信号量。
 *
 * \param sem 信号量实体, 与 AW_SEMB_INIT(), AW_SEMB_TAKE(), AW_SEMB_GIVE() 和
 *        aw_semb_terminate() 一起使用。
 *
 * \par 范例
 * 见 AW_SEMB_DECL()
 *
 ...
 *
 * \param sem           信号量实体,由 AW_SEMB_DECL()或 AW_SEMB_DECL_STATIC()定义
 * \param initial_state 初始状态:
 *                          - #AW_SEM_EMPTY
 *                          - #AW_SEM_FULL
 * \param options       初始化选项标志:
 *                          - #AW_SEM_Q_FIFO
 *                          - #AW_SEM_Q_PRIORITY
 *
 ...
 *     可以通过返回值来判断调用结果:AW_OK -成功获取到信号量；-ENXIO -信号量已经
 *     被摧毁
 *
 *   - 为 #AW_SEM_NO_WAIT 时, 调用不会被阻塞, 立即返回, 通过返回值来判断调用结果;
 *     AW_OK -成功获取到信号量; -EAGAIN -未获取到信号量,稍后再试; -ENXIO -信号量
 *     已经被终止
 *
 *   - 正数值为超时时间, 此时调用有超时地阻塞, 在设定的超时时间到达时, 若还未获
 *     取到信号量, 则调用返回, 可以通过返回值来判断调用结果： AW_OK -成功获取到
 *     信号量; -ETIME -超时,未获取到信号量,稍后再试; -ENXIO -信号量已经被终止
 *
 *   - 其它负数值为非法参数
 *
 * \param   sem      信号量实体,由 AW_SEMB_DECL() 或 AW_SEMB_DECL_STATIC() 定义
 ...
 * \retval  AW_OK   成功
 * \retval  -EINVAL 参数无效
 * \retval  -EPERM  不允许的操作,在中断中调用了本函数
 * \retval  -ETIME  获取信号量超时
 * \retval  -ENXIO  信号量已经被终止
 * \retval  -EAGAIN 获取信号量失败, 稍后再试 (\a timeout = #AW_SEM_NO_WAIT)
 *
 * \par 范例
 ...
 *  if (ret == -ETIME) {                            // 等待信号量超时
 *      //...
 *  }
 *
 *  aw_err_t ret;
 *  ret = AW_SEMB_TAKE(sem, AW_SEM_NO_WAIT);        // 无等待
 *  if (ret == -EAGAIN) {                           // 未获取到信号量
 *      //...
 *  }
 * \endcode
 ...
 * \brief 终止二进制信号量
 *
 * 终止信号量。任何等待此信号量的任务将会解阻塞, 并返回 -ENXIO 。此函数不会释放
 * 信号量相关的任何内存。
 *
 * \param   sem     信号量实体,由 AW_SEMB_DECL() 或 AW_SEMB_DECL_STATIC() 定义
 *
 ...
 *
 * \param   sem_id   信号量ID,由 AW_SEMB_INIT()返回
 * \param   timeout  最大等待时间(超时时间), 单位是系统节拍, 系统时钟节拍频率
 *                   由函数 aw_sys_clkrate_get()获得, 可使用 aw_ms_to_ticks()
 *                   将毫秒转换为节拍数; #AW_SEM_NO_WAIT 为无阻塞;
 *                   #AW_SEM_WAIT_FOREVER 为一直等待, 无超时
 ...
 * 信号量相关的任何内存。
 *
 * \param   sem_id  信号量实体,由 AW_SEMB_DECL() 或 AW_SEMB_DECL_STATIC() 定义
 *
 * \retval  AW_OK   成功
 * \retval  -EINVAL 参数无效
 *
 ...
 *
 * \param sem 信号量实体, 与 AW_SEMC_INIT(), AW_SEMC_TAKE(), AW_SEMC_GIVE()
 ...
 * 或先进先出的方式排队, 它们分别对应 #AW_SEM_Q_PRIORITY 和 #AW_SEM_Q_FIFO 。
 *
 * \param  sem          信号量实体,由 AW_SEMC_DECL()或 AW_SEMC_DECL_STATIC()定义
 * \param  initial_count  初始计数值, 非负数
 * \param  options        初始化选项标志:
 *                              - #AW_SEM_Q_FIFO
 *                              - #AW_SEM_Q_PRIORITY
 *
 ...
 *
 * \param   sem_id   信号量ID, 由 AW_SEMC_INIT()返回
 * \param   timeout  最大等待时间(超时时间), 单位是系统节拍, 系统时钟节拍频率
 *                   由函数 aw_sys_clkrate_get()获得, 可使用 aw_ms_to_ticks()
 *                   将毫秒转换为节拍数; #AW_SEM_NO_WAIT 为无阻塞;
 *                   #AW_SEM_WAIT_FOREVER 为一直等待, 无超时
 ...
 * \retval  -ETIME  获取信号量超时
 * \retval  -ENXIO  信号量已经被终止
 * \retval  -EAGAIN 获取信号量失败, 稍后再试 (timeout = AW_SEM_NO_WAIT)
 *
 * \par 范例
 ...
 *
 * \param sem 信号量实体, 与 AW_MUTEX_INIT(), AW_MUTEX_LOCK(), AW_MUTEX_UNLOCK()
 *            和 AW_MUTEX_TERMINATE() 一起使用。
 *
 * \par 范例：常规用法, 保护资源的访问
 * \code
 *  #include "aw_sem.h"
 ...
 * \endcode
 *
 * \sa AW_MUTEX_DECL_STATIC(), AW_MUTEX_INIT(), AW_MUTEX_LOCK(), AW_MUTEX_UNLOCK,
 *     AW_MUTEX_TERMINATE()
 ...
 * \endcode
 *
 * \sa AW_MUTEX_DECL(), AW_MUTEX_DECL_STATIC(), AW_MUTEX_INIT(), AW_MUTEX_LOCK(),
 ...
 *
 * \param  sem      信号量实体,由 AW_MUTEX_DECL()或 AW_MUTEX_DECL_STATIC()定义
 * \param  options  初始化选项标志("()"表示可选):
 ...
 *
 * \sa AW_SEM_Q_FIFO, AW_SEM_Q_PRIORITY
 * \sa AW_MUTEX_DECL(), AW_MUTEX_DECL_STATIC(), AW_MUTEX_LOCK(), AW_MUTEX_UNLOCK(),
 *     AW_MUTEX_TERMINATE()
 * \sa aw_mutex_lock(), aw_mutex_unlock(), aw_mutex_terminate()
 * \hideinitializer
 */
 ...
 * \endcode
 *
 * \sa AW_MUTEX_DECL(), AW_MUTEX_DECL_STATIC(), AW_MUTEX_INIT(), AW_MUTEX_UNLOCK(),
 *     AW_MUTEX_TERMINATE()
 * \sa aw_mutex_lock(), aw_mutex_unlock(), aw_mutex_terminate()
 * \hideinitializer
 */
...
#define AW_SEL_WRITE                    AW_PSP_SEL_WRITE
#define AW_SEL_READ                     AW_PSP_SEL_READ
