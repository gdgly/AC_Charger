...
/** \brief 用户无需直接使用该函数，应该使用宏AW_CRC_INIT()初始化 */
aw_err_t aw_crc_init(struct aw_crc_client *p_crc_client,void *crc_table,uint32_t flags);

/** \brief 用户无需直接使用该函数，应该使用宏AW_CRC_CAL()进行计算 */
void aw_crc_cal(struct aw_crc_client *p_crc_client,uint8_t *p_data,uint32_t nbytes);

/** \brief 用户无需直接使用该函数，应该使用宏AW_CRC_FINAL()得到最终的CRC计算结果 */
 ...
 *                  - AW_CRC_FLAG_CREATETAB   指定需要创建crc表，当系统不支持指定CRC模型时，
 *                                            传入此标志，将会自动将crctable填充正确的值。
 * \retval   AW_OK         初始化成功
 ...
 * \brief  CRC计算
 *
 * \param crc        CRC模型实体，由 AW_CRC_DECL() 或 AW_CRC_DECL_STATIC() 定义的
 * \param pdata      指向待进行CRC计算的数据
 * \param nbytes     本次计算的个数
 *
 * \return 无返回值
 *
