 ...
 *
 * AW_INT_CPU_UNLOCK(key0);    //CPU的中断继续保持关闭
 * AW_INT_CPU_UNLOCK(key1);    //CPU的中断继续保持关闭
 * AW_INT_CPU_UNLOCK(key2);    //CPU的中断被立即开启
 * \endcode
 *
 ...
 *
 * \retval      AW_OK       成功
 * \retval     -ENXIO       \a inum 为不存在的中断号
 * \retval     -EFAULT      \a pfunc_isr 为 NULL
 * \retval     -ENOSPC      空间不足
 *
 * \par 示例：常规用法
 * \code
 ...
 * \retval      AW_OK       成功
 * \retval     -ENXIO       \a inum 为不存在的中断号
 * \retval     -ENOENT      未找到参数为 \a p_arg 的 \a pfunc_isr
 *
 * \see aw_int_connect()
 */
