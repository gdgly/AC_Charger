...
typedef struct aw_spi_message {
    struct aw_list_head transfers;          /**< \brief 组成消息的传输段 */

    struct aw_spi_device *p_spi_dev;        /**< \brief 发送消息的SPI从设备 */
...
typedef struct aw_spi_device {
     ...
     * \param p_dev
     */
    void      (*pfunc_cs)(struct aw_spi_device *p_dev, int state);
 ...
 * \param busid         设备所属主机接口编号
 * \param bits_per_word 字大小，为0时使用默认的“8-bit字大小”
 * \param mode          设备模式标志，见“SPI模式标志”
 * \param max_speed_hz  设备支持的最大速度
 * \param cs_pin        片选引脚编号(\a pfunc_cs 为NULL时，本参数有效)
 * \param pfunc_cs      片选控制函数(本参数不为 NULL 时， \a cs_pin 参数无效)
 *
 * \par 范例
 * 见 aw_spi_setup()
 */
 ...
 * \brief 设置SPI传输结构体参数
 *
 * 参数的描述请参考 struct aw_spi_transfer
 *
 * \par 范例
 * 见 aw_spi_async()
 */
