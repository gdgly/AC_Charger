 ...
 * 2. 计数模式和定时模式；
 * 3. 2个32位捕获通道，可以在指定输入信号的边沿捕获定时器值，捕获事件同时还可以选择
 *   是否产生中断请求；
 ...
 *
 * \param[in] p_hw_timer : 指向Standard Timer寄存器块的指针
 * \param[in] ch         : 待清除中断标志的捕获通道，有效值 0 ~ 1
 *
 * \return 无
 */
am_static_inline 
...
{
    p_hw_timer->ccr = (p_hw_timer->ccr & 0x003F) | (1 << (ch * 3));
}
...
{
    p_hw_timer->ccr = (p_hw_timer->ccr & 0x003F) | (1 << (ch * 3 + 1));
}
...
{
    p_hw_timer->ccr = (p_hw_timer->ccr & 0x003F) | (1 << (ch * 3 + 2));
}
 ...
 * \param[in] cap_num    : 如果时钟源为捕获输入，则这里指定捕获通道号，有效值0~1
 *
 * \return 无
 */
am_static_inline  
void amhw_timer_src_set (amhw_timer_t     *p_hw_timer, 
                         ...
                         uint8_t           cap_num)
{
    p_hw_timer->ctcr = (p_hw_timer->ctcr &  0x0ul)  |
                       ((uint32_t)timer_src)        |
                       ((uint32_t)cap_num << 2);
}
