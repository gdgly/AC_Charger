 ...
 * - 1.00 15-08-07  cod, first implementation.
 * \endinternal
 */
...
/** IAP 的ROM API函数类型定义 */
typedef void (*amhw_iap_entry_t)(uint32_t[], uint32_t[]);
 ...
 */
am_static_inline
void amhw_iap_entry (uint32_t cmd_param[], uint32_t status_result[])
{
 ...
 * \param[in] nbytes   : 复制的数据个数, 应该是256/512/1024/4096
 ...
 * \param[in] nbytes   : 数据比较的个数,必须能被4整除
 *
 * \return IAP指令执行的状态
 *
